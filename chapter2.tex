...
This approach is fundamentally flawed, allowing the plaintext password to leave your device gives attackers many more opportunities to steal your password.
...
With \glspl{pake} your password never leaves your device, it is used to calculate a secret key which is shared with the server.
Another property of how \glspl{pake} are constructed is that you and the server are both \enquote{authenticated} when you acquire the shared key.
...
%     \node[scale=0.05] at (2,1) {\includegraphics{key.png}};
%     \node[scale=0.05] at (8,1) {\includegraphics{key.png}};
% 
%     \node[alice,minimum size=3cm] at (0,-7) {Alice -- Client};
%     \node[bob,mirrored,minimum size=3cm] at (10,-7) {Bob -- Server};
%     \node[scale=0.05] at (2,-6) {\includegraphics{key.png}};
%     \node[scale=0.05] at (8,-6) {\includegraphics{shuffled_key.png}};
% 
%     \draw (-2,-3.5) -- (12,-3.5);
...
In their paper \citetitle{quantum-annoying} \cite{quantum-annoying}, \citeauthor{quantum-annoying} define the \enquote{quantum annoying} property of \glspl{pake} as ``a scheme where a quantum computer can compromise a scheme, but only by solving one discrete logarithm for each guess of a password''.
This means that instead of only having to solve one discrete logarithm problem, it must be solved many times as the attackers guess many different passwords.
As solving just one discrete logarithm problem is expected to take a long time for early quantum computers, incorporating a large password space should make attacking \glspl{pake} take a prohibitively long amount of time -- millions of years or more for a large password space.
For example if we assume it takes ten seconds to solve a discrete logarithm problem for an early quantum computer, then an alphanumeric password of just ten characters would take roughly 133,070,041,519 years to crack, or roughly one hundred and thirty billion years.
This makes \glspl{pake} incredibly attractive as stop-gap solution giving researchers a few more vital years to research and test post-quantum cryptography.
...
These savings come from the fact there is no way
...
This now looks very different compared to when we were looking at them in $\RR$, however it shows very clearly what the elements of our set look like.
...
An additional unique bonus of this protocol as well is it takes into account the real-world issue of patents and aims to provide a practical protocol which is free from patents so as to promote the widest possible adoption.
...
After a conversation with the RustCrypto core team on Zulip, it was agreed that an implementation of any of the protocols from \cref{tab:modern-pakes} would be readily accepted into their collection of \glspl{pake} -- \url{https://github.com/RustCrypto/PAKEs}.
...
\gls{aucpace} also targets a rapidly growing area. The combination of these two factors means that means that \gls{aucpace} is uniquely positioned to make a large difference to the security of the \gls{iiot} landscape.
Additionally, it is the only protocol which is not under patent of any kind.
...
Having chosen to implemented \gls{aucpace} it is worth going over the protocol itself to understand better what implementing it will entail.
...
\begin{center}
  ...
  \begin{tabularx}{\linewidth}{ p{0.14\linewidth}XX }
    ...
    partial \newline augmentation & removes an expensive exponentiation operation for the server,
                                    halving the computational complexity
...
RustCrypto is a GitHub Organisation / online community which is dedicated to implementing fast and secure cryptography in pure Rust.
